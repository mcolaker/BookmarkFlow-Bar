...
- Kök neden ve kanıt: OpenAI Codex for Open Source programı aktif açık kaynak projelerin birincil veya çekirdek bakım sorumlularını; anlamlı kullanım, ekosistem önemi ve sürekli bakım kanıtlarıyla değerlendiriyor. Depo herkese açık, 27 commit, iki sürüm, CI, güvenlik politikası ve katkı akışı içeriyor; 2026-08-02 tarihli canlı GitHub kanıtında 3 yıldız, 2 izleyici, 0 fork, 0 issue ve sürüm varlıklarında 0 indirme görüldüğünden dış kullanım ve bakım etkinliği henüz güçlü biçimde kanıtlanmış değildir. Hak sahibi aynı tarihte `Apache-2.0 + ayrı marka politikası + inbound=outbound/DCO` modelini açıkça onayladı. Çalışma ağacında resmî Apache-2.0 ve DCO 1.1 metinleri, `NOTICE`, marka, yönetişim, yol haritası, destek ve katkı sözleşmeleri; PR commit aralığını fail-closed denetleyen CI; 14/14 binary varlık için boyut/SHA-256 köken kaydı ve sentetik verilerle yeniden üretilmiş beş pazarlama görseli hazırlandı. ImgBot commit'inin `main`, `v0.1.35` veya `v0.1.36` atası olmadığı ve güncel 11 varlık blobunun ImgBot öncesi sahip olunan sürümlerle aynı olduğu doğrulandı. Yerel açık kaynak kontratı 14/14, backlog kontratı 5/5, manifest `0.1.37` proje doğrulaması, 80 dosyalık public-tree ve İngilizce/Türkçe gerçek Chrome güvenlik regresyonları temizdir. Değişiklikler henüz GitHub'a yayımlanmadı; zorunlu yayın akışının önkoşulu olan GitHub CLI bu makinede kurulu değildir ve dış araç kurulumu kullanıcı onayı gerektirir.
...
- Sonraki adım: Kullanıcı onay verirse GitHub CLI'ı resmî kaynaktan kurup kimlik doğrulamasını doğrula; yalnız bu kapsamı DCO imzalı `agent/open-source-v0.1.37` dalına commit et, PR aç, `validate` işinin terminal `success` sonucunu bekle ve korumalı `main` dalına birleştir. Ardından `v0.1.37` etiketi ve doğrulanmış ZIP/SHA-256 sürümünü yayımla; açık ImgBot PR #2'yi birleştirmeden kapatıp uzak `imgbot` dalını sil; GitHub'ın `Apache-2.0` tanımasını ve Pages bağlantılarını canlı doğrula. Organik mağaza kullanımı, indirme ve dış issue/PR/Discussion kanıtı oluştuğunda doğru ölçümlerle programa başvur; seçim ve fayda etkinleşmesi harici kanıt gelene kadar görev açık kalır.
- Son güncelleme: 2026-08-02.
