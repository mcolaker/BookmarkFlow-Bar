...
- Sonraki adım: GitHub `About` homepage ayarını `https://mcolaker.github.io/BookmarkFlow-Bar/` olarak güncelle, imzasız repo sayfasından bağlantıyı açıp HTTPS `200` kanıtı al ve görevi yeniden `DONE` yap. Chrome Web Store dashboard URL uzlaştırması `BF-STORE-001` kapsamında açık tutulur.
- Son güncelleme: 2026-08-02.
...
- Doğrulama kapısı: Backlog, açık kaynak, DCO, proje ve public-tree doğrulamaları temizdir; `git diff --check` geçer. İmzasız GitHub repo sayfası yeni README rotalarını ve sosyal önizleme meta verisini; Discussions en az üç gerçek gönderiyi ve sabitlenmiş karşılama kaydını; Issues iki açık katkı girişini; profil ise bio ve `Pinned` BookmarkFlow Bar kartını gösterir. Repo ayarı `web_commit_signoff_required: true` döndürür; PR doğrulaması terminal `success` olur ve korumalı `main` dalına birleştirilir.
- Sonraki adım: README değişikliğini dar kapsamlı PR ile yayımla; ardından repo metadata, sosyal önizleme, Discussions, katkı issue'ları, profil sabitlemesi ve web commit sign-off ayarlarını uygula ve imzasız görünümde yeniden denetle.
- Son güncelleme: 2026-08-02.
