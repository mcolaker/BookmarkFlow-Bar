...
The extension also overrides Chrome's new tab page with a local extension page that includes a Google search field and the user's BookmarkFlow bookmark strip. The new tab bookmark strip remembers its horizontal scroll position locally.
...
The custom new tab search field navigates to Google Search only when the user submits a query.
...
`bookmarks` is required to render bookmarks and to create, move, or delete bookmarks only after explicit user actions. `storage` is required for preferences. `favicon` is required for bookmark favicons. The content script is required to render the overlay on websites.
